...
namespace DevAge.Patterns
{
	...
	public abstract class ActivityBase : IActivity
	{
		...
		/// <summary>
		/// An activity count used only to generate a sequential name
		/// </summary>
		/// <remarks>The count is incremented in a threadsafe manner across all threads.</remarks>
		private static int sActivityCount = 0; // Global, to make names unique across all threads.  Must use sLock.
	    private static readonly object sLock = new object(); // Lock for global changes to sActivityCount.
