...
{
	/// <summary>
	/// This controller dispatches each event to the specified cell.
	/// </summary>
	...
	{
	    [ThreadStatic] private static CellEventDispatcher t_Default;

	    /// <summary>
	    /// The default behavior of a cell.
	    /// </summary>
	    /// <remarks>This default CellEventDispatcher is shared across the same UI thread, but not between threads.</remarks>
	    public static CellEventDispatcher Default // Multi-thread safe with [ThreadStatic] backing store
	    {
	        get
	        {
                if (t_Default == null)
	                t_Default = new CellEventDispatcher();
