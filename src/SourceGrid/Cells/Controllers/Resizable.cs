...
namespace SourceGrid.Cells.Controllers
{
    ...
    {
        [ThreadStatic] private static Resizable t_ResizeBoth;
        [ThreadStatic] private static Resizable t_ResizeWidth;
        [ThreadStatic] private static Resizable t_ResizeHeight;

        /// <summary>
        /// Resize both width and height behavior
        /// </summary>
        /// <remarks>This provided Resizable controller is shared across the same UI thread, but not between threads.</remarks>
        public static Resizable ResizeBoth // Multi-thread safe with [ThreadStatic] backing store
        {
            get
            {
                if (t_ResizeBoth == null)
                    t_ResizeBoth = new Resizable(CellResizeMode.Both);
        ...
        /// <summary>
        /// Resize width only behavior
        /// </summary>
        /// <remarks>This provided Resizable controller is shared across the same UI thread, but not between threads.</remarks>
        public static Resizable ResizeWidth // Multi-thread safe with [ThreadStatic] backing store
        {
            get
            {
                if (t_ResizeWidth == null)
                    t_ResizeWidth = new Resizable(CellResizeMode.Width);

                return t_ResizeWidth;
            }
        }

        /// <summary>
        /// Resize height only behavior
        /// </summary>
        /// <remarks>This provided Resizable controller is shared across the same UI thread, but not between threads.</remarks>
        public static Resizable ResizeHeight // Multi-thread safe with [ThreadStatic] backing store
        {
            get
            {
                if (t_ResizeHeight == null)
                    t_ResizeHeight = new Resizable(CellResizeMode.Height);

                return t_ResizeHeight;
            }
        }
