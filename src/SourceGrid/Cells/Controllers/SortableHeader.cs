...
{
	/// <summary>
	/// A behavior that support sort and resize.
	/// </summary>
	/// <remarks>Once created cannot be modified. When calculated automatically the range to sort is all the grid range
	/// without the rows minor of the current row and the range header is all the grid range with the rows minor or equal
	/// of the current row.</remarks>
	public class SortableHeader : ControllerBase
	{
	    [ThreadStatic] private static SortableHeader t_Default;

	    /// <summary>
	    /// Column header behavior with sort and resize support, same as SortResizeHeader.
	    /// </summary>
        /// <remarks>This default SortableHeader is shared across the same UI thread, but not between threads.</remarks>
        public static SortableHeader Default // Multi-thread safe with [ThreadStatic] backing store
	    {
	        get
	        {
	            if (t_Default == null)
			        t_Default = new SortableHeader();
