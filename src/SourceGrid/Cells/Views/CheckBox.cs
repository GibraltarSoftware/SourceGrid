...
namespace SourceGrid.Cells.Views
{
	...
	{
	    [ThreadStatic] private static CheckBox t_Default; // Caches the default CheckBox used by each UI thread.
	    [ThreadStatic] private static CheckBox t_MiddleLeftAlign; // Caches a CheckBox with MiddleLeft alignment per UI thread.

	    /// <summary>
	    /// Represents a default CheckBox with the CheckBox image align to the Middle Center of the cell.
	    /// </summary>
	    /// <remarks>You must use this VisualModel with a Cell of type ICellCheckBox.  This default CheckBox is
	    /// shared across the same UI thread, but not between threads.  It can be cloned for a unique instance.</remarks>
	    public new static CheckBox Default // Multi-thead safe with [ThreadStatic] backing store
	    {
	        get
	        {
                if (t_Default == null)
	                t_Default = new CheckBox();
	    ...
	    /// </summary>
	    /// <remarks>This MiddleLeft-aligned CheckBox is shared across the same UI thread, but not between threads.
	    /// It can cloned for a unique instance.</remarks>
	    public static CheckBox MiddleLeftAlign // Multi-thead safe with [ThreadStatic] backing store
	    {
	        get
	        {
	            if (t_MiddleLeftAlign == null)
	            {
			        t_MiddleLeftAlign = new CheckBox();
			        t_MiddleLeftAlign.CheckBoxAlignment = DevAge.Drawing.ContentAlignment.MiddleLeft;
			        t_MiddleLeftAlign.TextAlignment = DevAge.Drawing.ContentAlignment.MiddleLeft;
	            }

	            return t_MiddleLeftAlign;
	        }
	    }
