...
namespace SourceGrid
{
	...
	public partial class Grid : GridVirtual
	{
		...
		#region Row/Col Span
        // Value types such as int variables can be safely shared between multiple UI threads...
        // provided they are readonly and not accessed by reference.  However, to support modification of these
        // default settings, it needs to be [ThreadStatic] and use some clever initialization logic, so that
        // it is *not* shared across mulitple threads (which would not be thread-safe in the way most
        // UI threads expect to operate).

	    private const int c_MaxSpan = 10; // Hard-coded global default value across all threads.

        // Since a MaxSpan of 0 isn't valid, we can key off that to determine if MaxSpan has been set to something.
        // If this is 0, it means the constant default should be taken as MaxSpan.  If this is greater than 0, it
        // means MaxSpan has been set to something specific, so this is the value of MaxSpan.  This avoids an extra
        // "initialization" step on every access.
        [ThreadStatic] private static int t_MaxSpan; // Starts as default int value of 0 for each thread.

        /// <summary>
        /// Gets the maximum rows or columns number to search when using Row/Col Span.
        /// </summary>
        /// <remarks><para>This is a ThreadStatic property shared across each thread, but not between threads.
        /// This is necessary to allow for multiple UI threads which need to operate independently.</para>
        /// <para>This value is automatically calculated based on the current cells. Do not change this value manually.
        /// (The previous statement needs to be rechecked; it does not appear to be adjusted automatically, and
        /// it does seem to support being changed at runtime.)  The initial default setting is 10.</para></remarks>
        public static int MaxSpan // Multi-thread safe with [ThreadStatic] backing store
        {
            get
            {
                return (t_MaxSpan > 0) ? t_MaxSpan : c_MaxSpan; // Use the constant unless this thread has changed MaxSpan.
            }
            set
            {
                t_MaxSpan = (value > 0) ? value : 1; // Set it to a sane value: 1 is the minimum meaningful MaxSpan.
            }
        }

		/// <summary>
		/// This method converts a Position to the real range of the cell.
		/// </summary>
		/// <remarks>This is usefull when RowSpan or ColumnSpan is greater than 1.
		/// For example suppose to have at grid[0,0] a cell with ColumnSpan equal to 2.
		/// If you call this method with the position 0,0 returns 0,0-0,1 and
		/// if you call this method with 0,1 return again 0,0-0,1.</remarks>
